...
const RenderHRDiagramContainer = ({data, syncId}) => {
    ...
    const [ data1, data2, yDomain, xDomain ] = useMemo(() => {
        ...
        const maxY = [...data1, ...data2]
            .reduce((acc, curr) => curr.Luminosity > acc ? curr.Luminosity : acc, 0);
        const minY = data2[0].Luminosity;

        const maxX = Math.ceil([...data1, ...data2]
            .reduce((acc, curr) => curr.Temperature > acc ? curr.Temperature : acc, 0));
        const minX = Math.floor([...data1, ...data2]
            .reduce((acc, curr) => curr.Temperature < acc ? curr.Temperature : acc, maxX));
    ...
    return (
        <RenderHRDiagram data1={data1} data2={data2} syncId={syncId} yDomain={yDomain} xDomain={xDomain}/>
    );
};
