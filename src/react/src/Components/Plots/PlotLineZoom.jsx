...
const drawText = ({ text: { meta, data }, xAxisMap, yAxisMap }) => {
    ...
    return (
        <text key={`${label}-${data.x}-${data.y}`} textAnchor="middle" fill={colour} x={pixX} y={pixY}>
            {label.map((token) => {
                if (token[0] === 'text') {
                    return <tspan key={token[1]}>{token[1]}</tspan>;
                } else if (token[0] === 'subscript') {
                    return (
                        <React.Fragment key={token[1]}>
            ...
            <ResponsiveContainer width="80%" height="100%">
                ...
                >
                    ...
                    {refLines.map((refLine) => (
                        <Customized key={refLine.meta.label} component={drawReferenceLine} refLineMeta={refLine} />
                    ))}
                    {texts.map((text) => (
