...
const PublicJobs = ({ data, match, router, relay }) => {
    const jobs =
        data && data.publicCompasJobs && data.publicCompasJobs.edges.length > 0
            ? data.publicCompasJobs.edges.map((e) => e.node)
            : null;
    ...
    return (
        <Container>
            <h1 className="pt-5 mb-4">Public Jobs</h1>
            <Form>
                <Form.Row>
                    <Col lg={6}>
                    ...
                    <Col lg={3}>
                        <Link to={'/job-form'} exact match={match} router={router} as={Button} variant="primary">
                            New Job
                        </Link>
                    </Col>
                </Form.Row>
                        ...
                        <Card key={job.id} className="job-card">
                            <Card.Body>
                                ...
                                <Card.Subtitle>
                                    ...
                                    . <span>{job.user}</span> . {job.timestamp}
                                </Card.Subtitle>
                                <Card.Text>{job.description}</Card.Text>
                                <Link
                                    to={'/job-results/' + job.id + '/'}
                                    exact
                                    match={match}
                                    router={router}
                                    as={Button}
...
export default createPaginationContainer(
    PublicJobs,
    {
        data: graphql`
            fragment PublicJobs_data on Query {
                publicCompasJobs(first: $count, after: $cursor, search: $search)
    ...
    },
    {
        direction: 'forward',
        query: graphql`
            query PublicJobsForwardQuery($count: Int!, $cursor: String, $search: String) {
                ...PublicJobs_data
            }
        `,
