...
const MyJobs = ({ data, match, router, relay }) => {
    const jobs =
        data && data.compasJobs && data.compasJobs.edges.length > 0 ? data.compasJobs.edges.map((e) => e.node) : null;
    ...
    return (
        <Container>
            <Row>
                <Col lg={6}>
                    <h1 className="pt-5 mb-3">My Jobs</h1>
                </Col>
                <Col lg={2} className="pt-5 mb-2">
                    <Link to={'/jobs/'} exact match={match} router={router} as={Button} variant="outline-primary">
                        Public Jobs
                    </Link>
                </Col>
                <Col lg={2} className="pt-5 mb-2">
    ...
    MyJobs,
    {
        data: graphql`
            fragment MyJobs_data on Query {
                compasJobs(first: $count, after: $cursor, orderBy: $orderBy) @connection(key: "MyJobs_compasJobs") {
                    edges {
                        node {
                            id
                            user
