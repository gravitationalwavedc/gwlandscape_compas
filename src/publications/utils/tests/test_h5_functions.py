...
class TestGetH5SubgroupMeta(TestCase):
    def setUp(self):
        ...
        self.subgroups = [
            'test_dataset_1', 'test_dataset_2', *(val for vals in self.default_subgroups.values() for val in vals)
        ]
    ...
    def test_returns_default_data_without_kwargs(self):
        with h5py.File(self.tf, 'w') as f:
            ...
            self.assertEqual(meta['subgroup_x'], self.default_subgroups.get(meta['group'], meta['subgroups'])[0])
            self.assertEqual(meta['subgroup_y'], self.default_subgroups.get(meta['group'], meta['subgroups'])[1])
    ...
    def test_returns_default_subgroups_for_each_root_group(self):
        with h5py.File(self.tf, 'w') as f:
            for root_group in self.default_subgroups.keys():
                ...
                self.assertEqual(meta['subgroup_x'], self.default_subgroups[meta['group']][0])
                self.assertEqual(meta['subgroup_y'], self.default_subgroups[meta['group']][1])
                self.assertEqual(meta['subgroup_x_unit'], f'{meta["subgroup_x"]}_unit')
                self.assertEqual(meta['subgroup_y_unit'], f'{meta["subgroup_y"]}_unit')
    ...
    def test_returns_subgroups_if_specified(self):
        with h5py.File(self.tf, 'w') as f:
            meta = get_h5_subgroup_meta(f, root_group='test_group', subgroup_x='Mass(1)', subgroup_y='Mass(2)')
            self.assertEqual(meta['group'], 'test_group')
...
class TestGetH5SubgroupData(TestCase):
    ...
    def test_returns_none_if_string_type(self):
        with h5py.File(self.tf, 'w') as f:
            self.assertIsNone(get_h5_subgroup_data(f, 'base_group', 'string_dataset', 'test_dataset'))
            self.assertIsNone(get_h5_subgroup_data(f, 'base_group', 'test_dataset', 'string_dataset'))
