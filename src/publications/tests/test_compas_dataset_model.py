...
class TestUploadModel(testcases.TestCase):
    ...
    @override_settings(MEDIA_ROOT=TemporaryDirectory().name)
    def test_get_run_details_archive(self):
        model = CompasDatasetModel.create_dataset_model(
            self.publication,
            self.model,
            self.test_job_archive
        ...
        run_details = model.get_run_details()
        self.assertTrue(run_details.count(), 1)
        self.assertTrue('Run_Details' in run_details.first().file.name)

    @override_settings(MEDIA_ROOT=TemporaryDirectory().name)
    def test_get_run_details_single(self):
        model = CompasDatasetModel.create_dataset_model(
            self.publication,
            self.model,
            self.test_job_single
        ...
        self.assertFalse(model.get_run_details().exists())

    @override_settings(MEDIA_ROOT=TemporaryDirectory().name)
    def test_get_data_archive(self):
        model = CompasDatasetModel.create_dataset_model(
            self.publication,
            self.model,
            self.test_job_archive
        ...
        self.assertTrue('.h5' in data.first().file.name)
    ...
    def test_get_data_single(self):
        ...
        self.assertTrue(model.get_data().exists())
