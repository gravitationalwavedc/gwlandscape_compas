...
class TestCompasDatasetModel(testcases.TestCase):
    def setUp(self):
        ...
        self.test_job_archive_multiple_h5 = SimpleUploadedFile(
            ...
            content=open(
                "./publications/tests/test_data/test_job_multiple_h5.tar.gz", "rb"
            ).read(),
            content_type="application/gzip",
        )
        ...
        self.test_job_not_h5 = SimpleUploadedFile(
            name="Run_Details",
            content=open(
                "./publications/tests/test_data/test_job/COMPAS_Output/Run_Details",
                "rb",
            ).read(),
    ...
    def test_create(self):
        CompasDatasetModel.create_dataset_model(
            self.publication, self.model, self.test_job_archive
        )

        self.assertEqual(CompasDatasetModel.objects.all().count(), 1)
        self.assertEqual(
            CompasDatasetModel.objects.last().compas_publication, self.publication
        )
    ...
    def test_update(self):
        ...
        new_model = CompasModel.create_model(
            "new test", "new summary", "new description"
        )
    ...
    def test_save_no_file(self):
        CompasDatasetModel.create_dataset_model(self.publication, self.model, None)
    ...
    def test_save_single_file(self):
        CompasDatasetModel.create_dataset_model(
            self.publication, self.model, self.test_job_single
        )
    ...
    def test_save_multiple_h5(self):
        with self.assertRaises(
            ...
            CompasDatasetModel.create_dataset_model(
                self.publication, self.model, self.test_job_archive_multiple_h5
            )
    ...
    def test_save_non_h5(self):
        with self.assertRaises(
            ...
            CompasDatasetModel.create_dataset_model(
                self.publication, self.model, self.test_job_not_h5
            )
