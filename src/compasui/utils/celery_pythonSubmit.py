...
class pythonProgramOptions:
    ...
    chemically_homogeneous_evolution = (
        'PESSIMISTIC'  # chemically homogeneous evolution.  Options are 'NONE', 'OPTIMISTIC' and 'PESSIMISTIC'
    )
    ...
    common_envelope_allow_main_sequence_survive = (
        True  # Allow main sequence stars to survive CE. Was previously False by default
    )
    ...
    kick_magnitude_random = (
        None  # (SSE) used to draw the kick magnitude for the star should it undergo a supernova event
    )
    ...
    kick_magnitude_random_1 = (
        None  # (BSE) used to draw the kick magnitude for the primary star should it undergo a supernova event
    )
    kick_magnitude_1 = (
    ...
    muller_mandel_kick_multiplier_NS = (
        400.0  # scaling prefactor for NS kicks when using the 'MULLERMANDEL' kick magnitude distribution
    )
    ...
    def listChoices(self):
        listChoices = [self.log_classes, self.debug_classes]

        return listChoices

    def listCommands(self):
        listCommands = ['--log-classes', '--debug-classes']
