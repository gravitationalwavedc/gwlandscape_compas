    ...
    for ii, specificPlot in enumerate(listOfPlots):  # exclude the last one
        ...
        if ii in [0, 1, 2]:  # top plots
            ...
            for jj in range(num_events):
                ...
                # The unicode representation of the capital letters -
                # works as long as there are less than 26 images to show
                ax.text(x=spaced_out_event_times[jj], y=ax.get_ylim()[1] * yOffsetFactor, s=chr(ord('@') + 1 + jj))
...
def plotLengthAttributes(ax=None, Data=None, mask=None, **kwargs):
    ...
    ax.plot(Data['Time'][()][mask1], Data['Radius(1)'][()][mask1] / Data['Radius(1)|RL'][()][mask1], linestyle='--',
            c='r', label='Roche Radius 1')
    mask2 = mask & (Data['Radius(2)|RL'][()] != 0)
    ax.plot(Data['Time'][()][mask2], Data['Radius(2)'][()][mask2] / Data['Radius(2)|RL'][()][mask2], linestyle='--',
            c='b', label='Roche Radius 2')
...
def plotStellarTypeAttributes(ax=None, Data=None, mask=None, **kwargs):
    ...
    ax.plot(Data['Time'][()][mask], typeNameMap(Data['Stellar_Type(1)'][()][mask]), linestyle='-', c='r',
            label='Stellar Type 1')
    ax.plot(Data['Time'][()][mask], typeNameMap(Data['Stellar_Type(2)'][()][mask]), linestyle='-', c='b',
...
def plotHertzsprungRussell(ax=None, Data=None, events=None, mask=None, **kwargs):
    ...
    for jj in range(np.sum(mask2)):
        # The unicode representation of the capital letters - works as long as there are less than 26 images to show
        ax.text(x=Tmsk[jj], y=Lmsk[jj], s=chr(ord('@') + 1 + jj))
