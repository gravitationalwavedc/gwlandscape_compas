...
def main(data_path, plot_path):
    ...
    axes[0][0].add_artist(
        Legend(
            axes[0][0], lines[4:], axes[0][0].get_legend_handles_labels()[1][4:], prop={'size': 8}, loc=(0.05, 0.15)
        )
    )
    ...
    axes[0][1].plot(
        Data['Time'][()],
        Data['Radius(1)'][()] / Data['Radius(1)|RL'][()],
        linestyle='--',
        c='r',
    ...
    axes[0][1].plot(
        Data['Time'][()],
        Data['Radius(2)'][()] / Data['Radius(2)|RL'][()],
        linestyle='--',
        c='b',
    ...
    axes[1][1].plot(
        Data['Time'][()], typeNameMap(Data['Stellar_Type(1)'][()]), linestyle='-', c='r', label='Stellar Type 1'
    )
    axes[1][1].plot(
        Data['Time'][()], typeNameMap(Data['Stellar_Type(2)'][()]), linestyle='-', c='b', label='Stellar Type 2'
    )
    ...
    axes[0][0].text(axes[0][0].get_xlim()[1] * -0.18, axes[0][0].get_ylim()[1] * 0.87, 'a)', fontweight='bold')
    axes[0][1].text(
        axes[0][1].get_xlim()[1] * -0.25, axes[0][1].get_ylim()[1] ** 0.75, 'b)', fontweight='bold'
    )  # exponent because it's on a log axis
    axes[1][0].text(axes[1][0].get_xlim()[1] * -0.18, axes[1][0].get_ylim()[1] * 0.87, 'c)', fontweight='bold')
    axes[1][1].text(axes[1][1].get_xlim()[1] * -0.25, axes[1][1].get_ylim()[1] * 0.87, 'd)', fontweight='bold')
