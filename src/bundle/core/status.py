...
def status(details, job_data):
    ...
    if not job:
        ...
        return dict({"status": result, "complete": True})
    ...
    sid_file = os.path.join(
        job["working_directory"], job["submit_directory"], "slurm_ids"
    )

    # Check if the slurm_ids file exists
    if not os.path.exists(sid_file):
        return dict({"status": status, "complete": False})
    ...
    for _sid in slurm_ids:
        ...
        status.append({"what": what, "status": _status, "info": info})
    ...
    return dict({"status": status, "complete": True})
